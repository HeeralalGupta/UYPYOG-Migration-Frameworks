...
public class PurchaseOrderServiceClient {
	...
	@Value("${purchaseorder.search}")
	private String purchaseOrderSearch;

	public PurchaseOrderServiceClient(RestTemplate restTemplate) {
	 ...
	 */
	public List<EgBillPurchaseItemsDTO> getPurchaseItemsByOrderNumber(String orderNumber, RequestInfo requestInfo,
			String tenantId) {
		...
		if (orderNumber == null || orderNumber.trim().isEmpty()) {
			throw new IllegalArgumentException("Purchase Order number is empty.");
		}
		...
		String url = financeHost + purchaseOrderSearch + "?tenantId=" + encodedTenantId + "&auth_token=" + encodedToken
		...
		System.out.println("====================================");
		System.out.println("PURCHASE ORDER API CALL");
		...
		try {
			...
			throw new RuntimeException("Purchase Order API call failed for order number: " + orderNumber, e);
		}
		...
		System.out.println("PURCHASE ORDER API STATUS : " + response.getStatusCode());
		System.out.println("PURCHASE ORDER API RESPONSE:");
		...
		if (responseBody == null || responseBody.trim().isEmpty()) {
			throw new IllegalArgumentException("Empty response received for Purchase Order: " + orderNumber);
		}
		...
		try {
			 ...
			 */
			JsonNode itemsNode = findPurchaseItemsArray(root);
			if (itemsNode == null || !itemsNode.isArray()) {
				throw new IllegalArgumentException("No work items found in Purchase Order response for: " + orderNumber);
			}
			...
			List<EgBillPurchaseItemsDTO> workItems = new ArrayList<>();

			for (JsonNode item : itemsNode) {
				EgBillPurchaseItemsDTO workItem = mapWorkItem(item);
				if (workItem != null) {
	 ...
	 *
	 * workItems purchaseitems puurchaseOrderItems egBillPurchaseItemsDTO items
	 */
	private JsonNode findPurchaseItemsArray(JsonNode node) {
		...
		if (node.isObject()) {
			...
			while (fields.hasNext()) {
				...
				if (normalized.equals("purchaseitems") || normalized.equals("purchaseorderitems")
						|| normalized.equals("egbillpurchaseitemsdto") || normalized.equals("items")) {
				...
				JsonNode result = findPurchaseItemsArray(value);
		...
		if (node.isArray()) {
			for (JsonNode child : node) {
				JsonNode result = findPurchaseItemsArray(child);
				if (result != null) {
					return result;
				}
			}
	 ...
	 */
	private EgBillPurchaseItemsDTO mapWorkItem(JsonNode item) {

		if (item == null || !item.isObject()) {
			return null;
		}

		EgBillPurchaseItemsDTO dto = new EgBillPurchaseItemsDTO();
