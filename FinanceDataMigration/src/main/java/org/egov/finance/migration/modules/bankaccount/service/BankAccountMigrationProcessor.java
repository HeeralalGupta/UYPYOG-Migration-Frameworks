...
import org.egov.finance.migration.modules.bankaccount.reader.BankAccountExcelReader;
...
@Service
public class BankAccountMigrationProcessor extends AbstractMigrationProcessor {

	private final BankAccountExcelReader excelReader;
	private final BankAccountRequestBuilder requestBuilder;
	...
	protected MigrationResult doProcess(MigrationRequest request) {
		...
		job.setCurrentMessage("Excel read successfully. " + "Starting bank account migration...");
		...
		/*
		 * ============================================================ STEP 4 : PROCESS
		 * EACH BANK ACCOUNT INDEPENDENTLY
		 *
		 * ONE BANK ACCOUNT = ONE API REQUEST
		 * ============================================================
		 */
		...
		for (int i = 0; i < records.size(); i++) {
			...
			if (alreadyMigrated) {
				...
				result.setMessage("Bank Account already migrated.");
				...
				updateJobProgress(job, i + 1, records.size(), success, failed, skipped,
						"Bank Account " + (i + 1) + " of " + records.size() + " skipped - already migrated.");
			...
			try {
				...
				CreateBankAccountRequest bankAccountRequest = requestBuilder.build(record, request);

				if (bankAccountRequest == null) {

					throw new RuntimeException("Unable to build " + "BankAccountRequest.");
				}

				/*
				 * Create Bank Account through Finance API
				 */
				BankAccountResponse response = bankAccountApiClient.createBankAccount(bankAccountRequest);

				if (response == null) {

					System.out.println("Bank Account API " + "returned null response");

					result.setStatus(RecordStatus.FAILED);

					result.setMessage("Bank Account API " + "returned null response");
					...
					result.setMessage("Bank Account created successfully.");
			...
			updateJobProgress(job, i + 1, records.size(), success, failed, skipped,
					"Processing bank account " + (i + 1) + " of " + records.size());
		}
		...
		if (failed > 0) {
			...
			finalMessage = "Bank Account migration completed with " + failed + " failed record(s).";

		} else if (skipped > 0) {

			finalMessage = "Bank Account migration completed successfully. " + skipped
					+ " record(s) skipped as duplicate.";

		} else {

			finalMessage = "Bank Account migration completed successfully.";
		}
