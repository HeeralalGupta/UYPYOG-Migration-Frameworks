...
public class ContractorBillExcelReader {
     ...
     * O = Bill Type
     * P = Debit GL Code
     * Q = Debit Account Head
     * R = Debit Amount
     * S = Credit/Deduction GL Code
     * T = Credit/Deduction Account Head
     * U = Deduction Percentage
     * V = Credit Amount
     * W = Net Payable GL Code
     * X = Net Payable Credit Amount
     * Y = Blank
     *
    ...
    public List<ContractorBillRecord> read(String filePath) {
        ...
        try (InputStream inputStream =
            ...
            /*
             * =====================================================
             * GET CONTRACTOR BILL SHEET
             * =====================================================
             *
             * First try to find "Contractor Bill"
             * case-insensitively.
             *
             * If workbook contains only one sheet, use that sheet.
             * This supports the new single-sheet workbook where
             * the sheet is named "Sheet1" but contains Contractor Bill.
             */
            ...
            for (int i = 0;
                ...
                if ("Contractor Bill".equalsIgnoreCase(
                        workbook.getSheetName(i).trim())) {
            ...
            /*
             * Contractor Bill sheet not found.
             */
            if (sheet == null) {
            ...
            for (int rowIndex = DATA_START_ROW;
                 ...
                 * =================================================
                 *
                 * ULB Name is populated on the first row
                 * of every bill.
                 */
                ...
                /*
                 * =================================================
                 * READ CREDIT / DEDUCTION DETAIL
                 * =================================================
                 */

                EgBillDetails creditDetail =
                        readCreditDetail(row);
                ...
                EgBillDetails netPayableDetail =
                        readNetPayableDetail(row);

                if (netPayableDetail != null) {

                    currentRecord
                            .getNetPayableDetails()
                            .add(netPayableDetail);
                }
            ...
            throw new RuntimeException(
                    "Unable to read Contractor Bill Excel file.",
                    e);
        }
    ...
    private boolean isNewBillRow(Row row) {
         ...
         * Primary marker:
         *
         * ULB Name must be populated on the first row
         * of every bill.
         */

        return !getCellValue(
                row,
                COL_ULB_NAME).isEmpty();
    }
    ...
    private ContractorBillRecord createBillRecord(Row row) {
        ...
        ContractorBillRecord record =
                new ContractorBillRecord();

        record.setUlbName(
                getCellValue(row, COL_ULB_NAME));

        record.setBillDate(
                getDateValue(row, COL_BILL_DATE));

        record.setContractor(
                getCellValue(row, COL_CONTRACTOR));

        record.setWorkOrder(
                getCellValue(row, COL_WORK_ORDER));

        record.setFund(
                getCellValue(row, COL_FUND));

        record.setDepartment(
                getCellValue(row, COL_DEPARTMENT));

        record.setScheme(
                getCellValue(row, COL_SCHEME));

        record.setFundSource(
                getCellValue(row, COL_FUND_SOURCE));

        record.setFunction(
                getCellValue(row, COL_FUNCTION));

        record.setNarration(
                getCellValue(row, COL_NARRATION));

        record.setPartyBillNo(
                getCellValue(row, COL_PARTY_BILL_NO));

        record.setPartyBillDate(
                getDateValue(row, COL_PARTY_BILL_DATE));

        record.setPartyBillAmount(
                parseBigDecimal(
                        getCellValue(
                                row,
                                COL_PARTY_BILL_AMOUNT)));

        record.setBillType(
                getCellValue(row, COL_BILL_TYPE));
    ...
    private EgBillDetails readDebitDetail(Row row) {
        ...
        EgBillDetails detail =
                new EgBillDetails();

        detail.setGlcodeid(
                parseBigDecimal(glCode));

        detail.setDebitamount(
                parseBigDecimal(debitAmount));
    ...
    private EgBillDetails readCreditDetail(Row row) {
        ...
        String accountHead =
                getCellValue(
                        row,
                        COL_CREDIT_ACCOUNT_HEAD);

        String deductionPercentage =
                getCellValue(
                        row,
                        COL_DEDUCTION_PERCENTAGE);
    ...
    private EgBillDetails readNetPayableDetail(Row row) {
        ...
        String glCode =
                getCellValue(
                        row,
                        COL_NET_PAYABLE_GL_CODE);

        String amount =
                getCellValue(
                        row,
                        COL_NET_PAYABLE_AMOUNT);
